...
const app=fs.readFileSync(path.join(root,'src/network-site.js'),'utf8');for(const feature of ['Journey builder','Complete edition manifest','Find the right kind of guide','download-brief','print-studio','Open access & pricing'])assert.ok(app.includes(feature),'missing '+feature);assert.ok(!app.includes('innerHTML=r.'));assert.deepEqual(require('../src/discovery-core.js').durations,[7,10,21,30,40,60,90]);

...
from import_library_updates import load_library_updates
...
update_records, library_updates = load_library_updates(ROOT, sections)
extra_records.extend(update_records)
...
stats.update(updateRecords=len(update_records), suppliedJourneys=len(library_updates["journeys"]), suppliedTools=len(library_updates["tools"]))
data=dict(schemaVersion=1,date='2026-09-16',libraryUpdates=library_updates,sourceUrl='https://docs.google.com/spreadsheets/d/1UQwgmPLqny7JVyU5POeQrCGMjRqJ7FAy/edit',sections=sections,assets=assets,spokes=spokes,initiatives=initiatives,stats=stats,extraRecords=extra_records,intelligence=intelligence)
...
shell=(ROOT/'src/index.html').read_text(); css=(ROOT/'src/library-updates.css').read_text()+'\n'+(ROOT/'src/style.css').read_text()+'\n'+(ROOT/'src/campaign-builder.css').read_text()+'\n'+(ROOT/'src/workspace.css').read_text()+'\n'+(ROOT/'src/lulu-studio.css').read_text()+'\n'+(ROOT/'src/discovery.css').read_text()+'\n'+(ROOT/'src/workspace-experience.css').read_text(); app=(ROOT/'src/app.js').read_text(); core=(ROOT/'src/core.js').read_text(); app=app.replace('/* CATALOG_EXTENSIONS */',(ROOT/'src/catalog-extensions.js').read_text()); app=(ROOT/'src/library-updates-core.js').read_text()+'\n'+(ROOT/'src/library-updates-ui.js').read_text()+'\n'+(ROOT/'src/discovery-core.js').read_text()+'\n'+(ROOT/'src/discovery-ui.js').read_text()+'\n'+(ROOT/'src/workspace-core.js').read_text()+'\n'+(ROOT/'src/campaign-core.js').read_text()+'\n'+app.replace('/* CAMPAIGN_BUILDER */',(ROOT/'src/campaign-builder.js').read_text()).replace('/* SHARED_WORKSPACE */',(ROOT/'src/workspace.js').read_text()).replace('/* WORKSPACE_EXPERIENCE */',(ROOT/'src/workspace-experience.js').read_text()).replace('/* LULU_STUDIO */',(ROOT/'src/lulu-studio.js').read_text()+'\n'+(ROOT/'src/lulu-checkout.js').read_text())
...
(dist/'archive-fulltext.json.gz').write_bytes((ROOT/'data/archive-fulltext.json.gz').read_bytes())

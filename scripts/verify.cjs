...
const memoryCount=D.stats.memoryTitles||0, updateCount=D.stats.updateRecords||0, sectionCount=79+(fs.existsSync(path.join(root,'data/memory-catalog.json'))?1:0)+(updateCount?1:0);
const master=D.sections[1].rows.map((r,i)=>({...Object.fromEntries(D.sections[1].columns.map((h,j)=>[h,r[j]||''])),_section:1,_row:i}));
assert.equal(master.length,18569);assert.equal(D.sections.length,sectionCount);assert.equal(D.assets.length,1088);assert.equal(D.spokes.length,10);assert.equal(D.intelligence.length,23);
assert.equal(master.length+D.extraRecords.length,56853+memoryCount+updateCount);assert.equal(new Set([...master,...D.extraRecords].map(r=>r['Master ID'])).size,56853+memoryCount+updateCount);
assert.equal(D.sections.reduce((n,s)=>n+s.rows.length,0),62826+memoryCount+updateCount);
for(const a of D.assets)assert.ok(a.url.startsWith('https://github.com/BRETTLIFETOGETHER/claude-artifacts/blob/main/'));
assert.equal(D.extraRecords.filter(r=>r._memory).length,memoryCount);
assert.equal(D.extraRecords.reduce((n,r)=>n+(r._memory?.sourceCount??r._memory?.sources.length??0),0),D.stats.memorySourceOccurrences||0);

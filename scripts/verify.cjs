...
const memoryCount=D.stats.memoryTitles||0, sectionCount=79+(fs.existsSync(path.join(root,'data/memory-catalog.json'))?1:0);
...
assert.equal(master.length+D.extraRecords.length,56853+memoryCount);assert.equal(new Set([...master,...D.extraRecords].map(r=>r['Master ID'])).size,56853+memoryCount);
assert.equal(D.sections.reduce((n,s)=>n+s.rows.length,0),62826+memoryCount);
...
assert.equal(errors.length,0);assert.ok(nodes.app.innerHTML.includes((56853+memoryCount).toLocaleString('en-US')));

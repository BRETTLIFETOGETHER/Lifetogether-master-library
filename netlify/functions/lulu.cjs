...
exports.handler=async(event,context)=>{
 try{
  ...
  if(action==='validate-file'){const kind=body.kind;if(!['interior','cover'].includes(kind))throw new L.InputError('Choose interior or cover.');const payload={source_url:L.pdfUrl(body.source_url,'PDF'),pod_package_id:L.pod(body.pod_package_id),...(kind==='cover'?{interior_page_count:L.integer(body.page_count,1,3000,'Page count')}:{})};return response(201,await lulu(`/validate-${kind}/`,{method:'POST',body:payload}));}
  if(action==='cover-dimensions')return response(200,await lulu('/cover-dimensions/',{method:'POST',body:{pod_package_id:L.pod(body.pod_package_id),interior_page_count:L.integer(body.page_count,1,3000,'Page count'),unit:'inch'}}));

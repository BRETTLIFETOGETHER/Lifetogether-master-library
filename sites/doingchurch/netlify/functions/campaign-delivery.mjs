...
async function runDelivery({ campaigns, subscriptions, workspace, claim, record, send, enabled, now = Date.now() }) {
  const results = [];
  if (!enabled) return results;
  for (const c of campaigns) {
    if (c.status !== "scheduled") continue;
    const ws = await workspace(c.workspace);
    if (!ws || !["owner", "editor"].includes(ws.members[c.author]?.role)) continue;
    for (const m of c.messages) {
      if (Date.parse(m.at) > now) continue;
      for (const s of subscriptions.filter((x) => x.workspace === c.workspace && ws.members[x.user] && (c.channel === "email" ? x.emailOptIn : x.smsOptIn))) {
        if (!s[c.channel + "ConsentedAt"] || Date.parse(s[c.channel + "ConsentedAt"]) > Date.parse(m.at)) continue;
        const key = `${c.id}/${m.id}/${s.id}`;
        if (!await claim(key, { state: "processing", at: new Date(now).toISOString(), campaign: c.id, message: m.id, subscriber: s.id })) continue;
        try {
          const result = await send(c, m, s, key);
          await record(key, { state: "accepted", providerId: result.id, channel: c.channel, at: new Date(now).toISOString(), campaign: c.id, message: m.id, subscriber: s.id });
          results.push({ key, state: "accepted" });
        } catch (e) {
          await record(key, { state: "needs-review", error: String(e.message).slice(0, 200), channel: c.channel, at: new Date(now).toISOString(), campaign: c.id, message: m.id, subscriber: s.id });
          results.push({ key, state: "needs-review" });
        }
      }
    }
  }
...
// platform/functions/campaign-delivery.mjs
var config = { schedule: "*/15 * * * *" };
var campaign_delivery_default = async () => {
  ...
  for (const [prefix, target] of [["campaigns/", campaigns], ["subscribers/", subscriptions]]) {
    for await (const page of store.list({ prefix, paginate: true })) {
      for (const b of page.blobs) target.push(await store.get(b.key, { type: "json" }));
    }
  }
  const results = await runDelivery({ campaigns, subscriptions, enabled: true, workspace: (id) => platform.get("workspaces/" + id, { type: "json" }), claim: async (key, data) => (await store.setJSON("attempts/" + key, data, { onlyIfNew: true })).modified, record: (key, data) => store.setJSON("attempts/" + key, data), send: async (c, m, s, key) => {
    const latest = await store.get("campaigns/" + c.id, { type: "json" }), subscriber = await store.get("subscribers/" + s.id, { type: "json" });
    if (latest?.status !== "scheduled" || !subscriber?.[c.channel + "OptIn"]) throw Error("Canceled or unsubscribed before dispatch.");
    ...
    if (c.channel === "email") {
      ...
      r = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: "Bearer " + process.env.RESEND_API_KEY, "Content-Type": "application/json", "Idempotency-Key": key }, body: JSON.stringify({ from: process.env.CAMPAIGN_FROM_EMAIL, to: [s.email], subject: m.subject, text }), signal: AbortSignal.timeout(15e3) });
    } else {

...
var text = (v, n = 200) => String(v ?? "").trim().slice(0, n);
...
function createService(store, clock = () => /* @__PURE__ */ new Date()) {
  ...
  async function create(key, data) {
    const r = await store.setJSON(key, data, { onlyIfNew: true });
    if (!r.modified) fail(409, "Already created. Reload and try again.");
    return r;
  }
  async function write(key, data, etag) {
    const r = await store.setJSON(key, data, { onlyIfMatch: etag });
    if (!r.modified) fail(409, "Someone saved a newer version. Reload before saving your changes.");
    return r;
  }
  async function account(uid, wid) {
    const key = "accounts/" + uid;
    for (let i = 0; i < 4; i++) {
      const r = await read(key), data = r?.data || { workspaces: [] };
      if (data.workspaces.includes(wid)) return;
      data.workspaces.push(wid);
      const saved = await store.setJSON(key, data, r ? { onlyIfMatch: r.etag } : { onlyIfNew: true });
      if (saved.modified) return;
    }
    fail(409, "Workspace saved. Open its link to reconnect it to your account.");
  }
  async function workspace(wid, user) {
    const r = await read("workspaces/" + id(wid));
    if (!r || !r.data.members[user.id]) fail(404, "Workspace not found or access was removed.");
    return r;
  }
  function visible(doc, user, role) {
    return doc.visibility === "workspace" || doc.owner === user.id;
  }
  ...
  function validateDocument(input, old, user) {
    ...
    if (JSON.stringify(input.content).length > 15e4) fail(413, "This item is too large. Split it into smaller documents.");
    ...
    if (input.content.recipe === "reader") {
      const c = input.content;
      if (!Array.isArray(c.days) || !c.days.length || c.days.length > 90) fail(400, "A reading edition must contain 1\u201390 days.");
      if (c.status === "Approved internally" && (!c.reviewer || c.days.some((d) => !d.title || !d.scripture || !d.body))) fail(400, "Complete every reading and name the reviewer before approval.");
    }
  ...
  return async function service(action, body, user) {
    if (action === "shared") {
      const r2 = await read("workspaces/" + id(body.workspace));
      if (!r2) fail(404, "Shared brief unavailable.");
      const share = r2.data.shares.find((s) => !s.revoked && s.expiresAt > now() && s.tokenHash === hash(text(body.token, 150)));
      if (!share) fail(404, "This link expired or was revoked.");
      return { title: share.title, document: share.snapshot, expiresAt: share.expiresAt };
    }
    if (!user?.id) fail(401, "Sign in to open your shared workspace.");
    if (action === "list") {
      const a = await read("accounts/" + user.id);
      const workspaces = [];
      for (const wid of a?.data.workspaces || []) {
        const r2 = await read("workspaces/" + wid);
        if (r2?.data.members[user.id]) workspaces.push({ id: wid, name: r2.data.name, kind: r2.data.kind, role: r2.data.members[user.id].role, updatedAt: r2.data.updatedAt });
      }
      return { workspaces, user: { id: user.id, name: user.name, email: user.email } };
    }
    if (action === "create") {
      const name = text(body.name);
      if (!name || !kinds.has(body.kind)) fail(400, "Enter a name and workspace type.");
      const a = await read("accounts/" + user.id);
      if ((a?.data.workspaces.length || 0) >= 40) fail(400, "You have reached the workspace limit.");
      const wid = randomUUID(), w2 = { id: wid, name, kind: body.kind, createdAt: now(), updatedAt: now(), members: { [user.id]: { role: "owner", name: text(user.name || user.email) } }, documents: [], events: [], invites: [], shares: [] };
      const saved2 = await create("workspaces/" + wid, w2);
      await account(user.id, wid);
      return { workspace: view(w2, user), etag: saved2.etag };
    }
    if (action === "accept") {
      const wid = id(body.workspace), r2 = await read("workspaces/" + wid);
      if (!r2) fail(404, "Invitation unavailable.");
      const w2 = r2.data;
      const invite = w2.invites.find((s) => !s.revoked && !s.used && s.expiresAt > now() && s.tokenHash === hash(text(body.token, 150)));
      ...
      if (w2.members[user.id]) fail(409, "You already belong to this workspace.");
      w2.members[user.id] = { role: invite.role, name: text(user.name || user.email) };
      invite.used = now();
      w2.events.push({ type: "invitation_accepted", at: now(), actor: user.id });
      w2.updatedAt = now();
      const saved2 = await write("workspaces/" + wid, w2, r2.etag);
      await account(user.id, wid);
      return { workspace: view(w2, user), etag: saved2.etag };
    }
    const r = await workspace(body.workspace, user), w = r.data, role = w.members[user.id].role;
    if (action === "get") {
      await account(user.id, w.id);
      return { workspace: view(w, user), etag: r.etag };
    }
    ...
    if (action === "save") {
      ...
      const doc = validateDocument(body.document || {}, old, user);
      if (old) w.documents[w.documents.indexOf(old)] = doc;
      else {
        if (w.documents.length >= 300) fail(400, "This workspace has reached its document limit. Export or create another workspace.");
      ...
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || !roles.has(body.role)) fail(400, "Enter a valid email and role.");
      if (w.invites.filter((i) => !i.revoked && !i.used && i.expiresAt > now()).length >= 50) fail(400, "Remove an unused invitation first.");
      const token = randomBytes(32).toString("base64url"), inv = { id: randomUUID(), email, role: body.role, tokenHash: hash(token), expiresAt: new Date(clock().getTime() + 7 * 864e5).toISOString() };
      w.invites.push(inv);
      result.invitation = { id: inv.id, token, expiresAt: inv.expiresAt };
      w.events.push({ type: "invitation_created", at: now(), actor: user.id });
    } else if (action === "revoke-invite") {
      owner(w, user);
      const inv = w.invites.find((i) => i.id === body.id);
      if (!inv) fail(404, "Invitation not found.");
      inv.revoked = now();
    } else if (action === "member") {
      owner(w, user);
      if (body.id === user.id) fail(400, "The owner cannot remove or demote themselves.");
      if (!w.members[body.id]) fail(404, "Member not found.");
      if (body.role === "remove") delete w.members[body.id];
      else if (roles.has(body.role)) w.members[body.id].role = body.role;
      else fail(400, "Invalid role.");
    } else if (action === "share") {
      edit(w, user);
      const doc = w.documents.find((d) => d.id === body.id && d.visibility === "workspace");
      if (!doc || !["campaign", "service", "calendar", "tool", "ministry"].includes(doc.type)) fail(400, "Only a shared plan or tool can be shared by link.");
      if (body.confirm !== true) fail(400, "Review the exact document and confirm sharing first.");
      const token = randomBytes(32).toString("base64url"), s = { id: randomUUID(), title: doc.title, snapshot: { title: doc.title, type: doc.type, content: doc.content }, tokenHash: hash(token), expiresAt: new Date(clock().getTime() + 7 * 864e5).toISOString() };
      w.shares.push(s);
      result.share = { id: s.id, token, expiresAt: s.expiresAt };
    } else if (action === "revoke-share") {
      owner(w, user);
      const s = w.shares.find((x) => x.id === body.id);
      if (!s) fail(404, "Share not found.");
      s.revoked = now();
    } else if (action === "event") {
      if (role === "viewer") fail(403, "Read-only account.");
      if (!eventTypes.has(body.type)) fail(400, "Unsupported event.");
      w.events.push({ type: body.type, at: now(), actor: user.id, reference: text(body.reference, 80) });
    } else fail(400, "Unknown action.");
    w.events = w.events.slice(-2e3);
    w.updatedAt = now();
    if (JSON.stringify(w).length > 3e6) fail(413, "Workspace storage limit reached. Export and create another workspace.");
    const saved = await write("workspaces/" + w.id, w, r.etag);
    return { ...result, workspace: view(w, user), etag: saved.etag };
  };
}

// platform/functions/platform.mjs
var platform_default = async (request) => {
  ...
  try {
    ...
    if (raw.length > 3e5) throw new ApiError(413, "Request too large.");
    let body;
    try {
      body = JSON.parse(raw);
    } catch {
    ...
    const result = await service(body.action, body, user);
    return Response.json(result, { headers });
  } catch (error) {
    const status = Number(error.status) || 500;
    return Response.json({ error: status < 500 ? error.message : "The workspace could not be saved. Your current form is still available; please retry." }, { status, headers });
  }
};
export {
